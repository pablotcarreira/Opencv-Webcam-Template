...
class ClassifiedOutputScene(CameraOutputScene):
    ...
    def __init__(self, camera_device: CameraDevice, model: str, nome="Classificada"):
        super(ClassifiedOutputScene, self).__init__(camera_device, nome)
        self.classificador = keras.models.load_model(model)
        self.previsoes = []
    ...
    def _on_new_frame(self, array):
        ...
        # Prepara a matriz para o processamento.
        gray_array = cv2.cvtColor(array, cv2.COLOR_RGB2GRAY)
        mini_array = cv2.resize(gray_array, (32, 32), cv2.INTER_LINEAR)

        pixmap = QPixmap(QImage(gray_array, gray_array.shape[1], gray_array.shape[0], QImage.Format_Grayscale8))
        self.frame_graphics_item.setPixmap(pixmap)


        previsao = self.classificador.predict_classes([mini_array.reshape(-1,1,32,32)], verbose=0)
        print(previsao)
...
if __name__ == '__main__':
    ...
    cena2 = ClassifiedOutputScene(my_device, model="modelos/cifar_1.model", nome="Imagem Processada")
